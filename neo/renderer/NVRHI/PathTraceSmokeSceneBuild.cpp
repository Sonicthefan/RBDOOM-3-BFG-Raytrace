...
void PathTracePrimaryPass::BuildRayTracingSmokeTestScene(const viewDef_t* viewDef)
{
    ...
    uint64_t bvhGeometryContentSignature = 14695981039346656037ull;
    bvhGeometryContentSignature = HashSmokePlanBytes(bvhGeometryContentSignature, &staticSignature.hash, sizeof(staticSignature.hash));
    bvhGeometryContentSignature = HashSmokePlanBytes(bvhGeometryContentSignature, &geometryUniverseStats.generation, sizeof(geometryUniverseStats.generation));
    bvhGeometryContentSignature = HashSmokePlanBytes(bvhGeometryContentSignature, &dynamicVertexCount, sizeof(dynamicVertexCount));
    bvhGeometryContentSignature = HashSmokePlanBytes(bvhGeometryContentSignature, &dynamicIndexCount, sizeof(dynamicIndexCount));
    bvhGeometryContentSignature = HashSmokePlanBytes(bvhGeometryContentSignature, &rigidRouteBuild.stats.vertices, sizeof(rigidRouteBuild.stats.vertices));
    bvhGeometryContentSignature = HashSmokePlanBytes(bvhGeometryContentSignature, &rigidRouteBuild.stats.indexes, sizeof(rigidRouteBuild.stats.indexes));
    bvhGeometryContentSignature = HashSmokePlanBytes(bvhGeometryContentSignature, &rigidRouteBuild.stats.triangles, sizeof(rigidRouteBuild.stats.triangles));
    uint64_t bvhActiveSetSignature = staticActiveSetPlan.activeSetSignature;
    bvhActiveSetSignature = HashSmokePlanBytes(bvhActiveSetSignature, &rigidRouteBuild.stats.emittedInstances, sizeof(rigidRouteBuild.stats.emittedInstances));
    bvhActiveSetSignature = HashSmokePlanBytes(bvhActiveSetSignature, &rigidRouteBuild.stats.emittedSeenThisFrame, sizeof(rigidRouteBuild.stats.emittedSeenThisFrame));
    bvhActiveSetSignature = HashSmokePlanBytes(bvhActiveSetSignature, &rigidRouteBuild.stats.emittedFromCache, sizeof(rigidRouteBuild.stats.emittedFromCache));
    uint64_t bvhTlasInstanceSignature = 14695981039346656037ull;
    const uint32_t hasStaticBlasBit = hasStaticBlas ? 1u : 0u;
    const uint32_t hasDynamicBlasBit = hasDynamicBlas ? 1u : 0u;
    const int rigidTlasInstanceCount = static_cast<int>(rigidTlasRouteInstances.size());
    bvhTlasInstanceSignature = HashSmokePlanBytes(bvhTlasInstanceSignature, &bvhActiveSetSignature, sizeof(bvhActiveSetSignature));
    bvhTlasInstanceSignature = HashSmokePlanBytes(bvhTlasInstanceSignature, &hasStaticBlasBit, sizeof(hasStaticBlasBit));
    bvhTlasInstanceSignature = HashSmokePlanBytes(bvhTlasInstanceSignature, &hasDynamicBlasBit, sizeof(hasDynamicBlasBit));
    bvhTlasInstanceSignature = HashSmokePlanBytes(bvhTlasInstanceSignature, &instanceCount, sizeof(instanceCount));
    bvhTlasInstanceSignature = HashSmokePlanBytes(bvhTlasInstanceSignature, &rigidTlasInstanceCount, sizeof(rigidTlasInstanceCount));
    ...
    bvhDirtyInput.current.geometryContentSignature = bvhGeometryContentSignature;
    bvhDirtyInput.current.materialGeneration = materialTableSignature;
    bvhDirtyInput.current.activeSetSignature = bvhActiveSetSignature;
    bvhDirtyInput.current.tlasInstanceSignature = bvhTlasInstanceSignature;
    const RtSmokeBvhDirtyPlan bvhDirtyPlan = BuildSmokeBvhDirtyPlan(bvhDirtyInput);
    m_smokeBvhDirtyPreviousToken = bvhDirtyInput.current;
    ...
    sceneLogDesc.bvhActiveSetSignature = bvhActiveSetSignature;
    sceneLogDesc.bvhResidentSetSignature = staticActiveSetPlan.residentSetSignature;
    sceneLogDesc.bvhGeometryContentSignature = bvhGeometryContentSignature;
    sceneLogDesc.bvhTlasInstanceSignature = bvhTlasInstanceSignature;
