...
void PathTracePrimaryPass::BuildRayTracingSmokeTestScene(const viewDef_t* viewDef)
{
    ...
    const int requestedDebugMode = idMath::ClampInt(0, 29, r_pathTracingDebugMode.GetInteger());
    const bool enableTextureProbe = (requestedDebugMode >= 8 && requestedDebugMode <= 20) || requestedDebugMode == 26 || requestedDebugMode == 27 || requestedDebugMode == 28 || requestedDebugMode == 29;
