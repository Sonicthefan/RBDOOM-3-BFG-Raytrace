...
float4 PathTraceNeeCacheBuildCandidatesForCell(PathTraceNeeCacheCellDebug cell, uint view)
{
    ...
    float bestAnalyticSourcePdf = 0.0;
    float bestAnalyticInvSourcePdf = 0.0;
    uint bestAnalyticReplayValid = 0u;
    uint writtenCount = 0u;
    uint firstEmissiveRejectedType = 0u;
    uint zeroEnergyEmissiveRejectCount = 0u;
    uint firstAnalyticRejectedType = 0u;
    uint structurallyInvalidAnalyticRejectCount = 0u;
    [loop]
    for (uint slot = 0u; slot < candidateSlots; ++slot)
    {
        const uint slotIndex = baseSlot + slot;
        PathTraceNeeCacheCandidates[slotIndex] = PathTraceNeeCacheMakeEmptyCandidate(cell.cellIndex, slot);
        if (PathTraceNeeCacheSourceDomain() == 0u)
        {
            if (cacheableFullRangeCount == 0u)
            {
                continue;
            }

            const uint denseIndex = (cell.hash + slot * 2654435761u) % cacheableFullRangeCount;
            const PathTraceUnifiedLightRecord record = PathTraceRestirLightManagerCurrentPayload[denseIndex];
            const float sourcePdf = 1.0 / max((float)cacheableFullRangeCount, 1.0);
            if (record.type == PATH_TRACE_UNIFIED_LIGHT_TYPE_DOOM_ANALYTIC)
            {
                if (!PathTraceNeeCacheAnalyticStableCacheable(record))
                {
                    ++structurallyInvalidAnalyticRejectCount;
                    continue;
                }

                if (!PathTraceNeeCacheAnalyticStructurallyValid(record))
                {
                    ++structurallyInvalidAnalyticRejectCount;
                    continue;
                }

                const float candidateWeight = PathTraceNeeCacheAnalyticCandidateWeight(record, cell);
                ...
                candidate.denseRluIndex = denseIndex;
                candidate.lightClass = PATH_TRACE_UNIFIED_LIGHT_TYPE_DOOM_ANALYTIC;
                candidate.sourcePdf = sourcePdf;
                candidate.invSourcePdf = 1.0 / max(sourcePdf, 1.0e-8);
                candidate.candidateWeight = candidateWeight;
                candidate.cellIndex = cell.cellIndex;
                candidate.candidateSlot = slot;
                candidate.flags = 1u;
                PathTraceNeeCacheCandidates[slotIndex] = candidate;
                ++writtenCount;

                if (candidateWeight >= bestAnalyticWeight)
                {
                    bestAnalyticWeight = candidateWeight;
                    bestAnalyticDenseIndex = denseIndex;
                    bestAnalyticSourcePdf = sourcePdf;
                    bestAnalyticInvSourcePdf = candidate.invSourcePdf;
                    bestAnalyticReplayValid = PathTraceNeeCacheAnalyticRluReplayValid(denseIndex) ? 1u : 0u;
                }
                continue;
            }
            ...
            if (record.type == PATH_TRACE_UNIFIED_LIGHT_TYPE_EMISSIVE_TRIANGLE)
            {
                ...
                {
                    ++zeroEnergyEmissiveRejectCount;
                    continue;
                }

                const float candidateWeight = PathTraceNeeCacheEmissiveCandidateWeight(record, cell);
                PathTraceNeeCacheCandidateRecord candidate;
                candidate.denseRluIndex = denseIndex;
                candidate.lightClass = PATH_TRACE_UNIFIED_LIGHT_TYPE_EMISSIVE_TRIANGLE;
                candidate.sourcePdf = sourcePdf;
                ...
                if (candidateWeight >= bestEmissiveWeight)
                {
                    bestEmissiveWeight = candidateWeight;
                    bestEmissiveDenseIndex = denseIndex;
                    bestEmissiveSourcePdf = sourcePdf;
                }
                continue;
            }

            if (firstEmissiveRejectedType == 0u)
            {
                firstEmissiveRejectedType = record.type;
            }
            if (firstAnalyticRejectedType == 0u)
            {
                firstAnalyticRejectedType = record.type;
            }
            continue;
        }
        ...
        const bool chooseAnalytic =
            ...
            (activeEmissiveRangeCount == 0u || (slot & 1u) != 0u || view == 6u || view == 7u || view == 10u);
        const uint rangeCount = chooseAnalytic ? activeAnalyticRangeCount : activeEmissiveRangeCount;
        const uint rangeOffset = chooseAnalytic ? analyticRangeOffset : emissiveRangeOffset;
        if (rangeCount == 0u)
        {
            continue;
        }

        const uint localIndex = (cell.hash + slot * 2654435761u) % rangeCount;
        ...
        if (chooseAnalytic)
        {
            ...
            const float classProbability = PathTraceNeeCacheTypedClassProbability(true, activeEmissiveRangeCount, activeAnalyticRangeCount);
            ...
            candidate.lightClass = PATH_TRACE_UNIFIED_LIGHT_TYPE_DOOM_ANALYTIC;
            candidate.sourcePdf = sourcePdf;
            candidate.invSourcePdf = invSourcePdf;
            candidate.candidateWeight = candidateWeight;
            candidate.cellIndex = cell.cellIndex;
            candidate.candidateSlot = slot;
            candidate.flags = 1u;
            ...
            if (candidateWeight >= bestAnalyticWeight)
            {
                ...
                bestAnalyticDenseIndex = denseIndex;
                bestAnalyticSourcePdf = sourcePdf;
                bestAnalyticInvSourcePdf = invSourcePdf;
                bestAnalyticReplayValid = PathTraceNeeCacheAnalyticRluReplayValid(denseIndex) ? 1u : 0u;
            }
            continue;
        }
        ...
        const float classProbability = PathTraceNeeCacheTypedClassProbability(false, activeEmissiveRangeCount, activeAnalyticRangeCount);
        const float sourcePdf = classProbability * max(record.sourcePdf, 1.0 / max((float)rangeCount, 1.0));
        const float candidateWeight = PathTraceNeeCacheEmissiveCandidateWeight(record, cell);
        PathTraceNeeCacheCandidateRecord candidate;
        candidate.denseRluIndex = denseIndex;
        candidate.lightClass = PATH_TRACE_UNIFIED_LIGHT_TYPE_EMISSIVE_TRIANGLE;
    ...
    PathTraceNeeCacheCells[cell.cellIndex].candidateCount = writtenCount;
    PathTraceNeeCacheWriteProviderResult(cell);
    ...
    if (view == 6u)
    {
        if (bestAnalyticDenseIndex == 0xffffffffu)
        {
            if (firstAnalyticRejectedType != 0u)
            {
                return float4(0.70, 0.04 + 0.12 * ((firstAnalyticRejectedType & 3u) / 3.0), 0.02, 1.0);
            }
            if (structurallyInvalidAnalyticRejectCount != 0u)
            {
                return float4(0.22, 0.02, 0.01, 1.0);
            }
    ...
    if (bestEmissiveDenseIndex == 0xffffffffu)
    {
        if (firstEmissiveRejectedType != 0u)
        {
            return float4(0.70, 0.04 + 0.12 * ((firstEmissiveRejectedType & 3u) / 3.0), 0.02, 1.0);
        }
        if (zeroEnergyEmissiveRejectCount != 0u)
        {
            return float4(0.22, 0.02, 0.01, 1.0);
        }
        return float4(0.12, 0.03, 0.02, 1.0);
