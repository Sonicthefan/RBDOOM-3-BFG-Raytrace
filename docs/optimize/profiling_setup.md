...
| `r_pathTracingCleanRestirGiSpecularProducer` | 0 | Specular GI mode: 0 off, 1 full specular seed producer (`FirstIndirect.0d/0e` full-screen trace + shade), 2 fallback/reuse mode that keeps specular final-output eligibility but skips the extra full-screen specular seed producer. Mode 2 is the Remix-shaped A/B for avoiding a second specular first-indirect path. |
