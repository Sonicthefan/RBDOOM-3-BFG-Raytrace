...
| `r_pathTracingCleanRestirGiSpatialVisibility` | 1 | Diagnostic for `CleanGI.2 SpatialReuse`: 0 accepts spatial neighbor reuse samples without the per-neighbor visibility shadow ray, 1 validates every neighbor, 2 validates an alternating half of neighbor candidates and accepts the rest. Use this with the correct RT producer path to measure how much of spatial cost is visibility validation versus reservoir/light math. |
