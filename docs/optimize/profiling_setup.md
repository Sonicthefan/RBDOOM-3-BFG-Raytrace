...
| `r_pathTracingCleanRestirGiGlossySecondRay` | 0 | Mixed-producer experiment: when specular mode 2 is active, seed the GI INIT page with one extra specular first-indirect ray on glossy primary surfaces. A/B showed a severe framerate loss with no visible quality gain, so keep this off unless specifically reprofiling the two-ray idea. |
