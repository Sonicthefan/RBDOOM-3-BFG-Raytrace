...
| `r_pathTracingCleanRestirGiProducerLeanSplit` | 0 | Uses `CleanGI.0a IndirectProducerLeanTrace` + `CleanGI.0b IndirectProducerLeanShade`: a two-dispatch TraceRay baseline that keeps the simple producer's constrained-normal trace but splits it from a stripped one-sample shade. If this and `r_pathTracingCleanRestirGiProducerSimple` are both enabled, the lean split path wins. |
