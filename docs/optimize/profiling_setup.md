...
| `r_pathTracingCleanRestirGiSecondaryRluCandidates` | 2 | RLU RIS candidate count inside `FirstIndirect.0b ShadeFast` for each first-secondary direct-light proposal. `8` matches the old DI-candidate-count behavior; use `1/2/4/8` to measure shade cost vs. noise. |
...
| `r_pathTracingCleanRtxdiDiTemporalFireflyClamp` | 32 | Clean DI temporal history outlier guard. `0` disables it; otherwise previous reservoirs older than one frame are rejected when their `targetPdf * W` contribution scale exceeds the current selected reservoir by this ratio. This is meant for `r_cleanDiSpatialSamples 1` / temporal-heavy testing, so firefly history is not repeatedly re-packed before DLSS RR sees it. |
| `r_pathTracingCleanRestirGiSpecularProducer` | 2 | Specular GI mode: 0 off, 1 full specular seed producer reference (`FirstIndirect.0d/0e` full-screen trace + shade), 2 one-ray mixed producer mode. Mode 2 keeps specular final-output eligibility, skips the extra full-screen specular seed producer, and lets the normal `FirstIndirect.0a` path sample a diffuse/specular mixture. Modes 1 and 2 both quarantine low-roughness/high-F0 receivers from GI temporal/spatial reuse to avoid reflection smearing. |
| `r_pathTracingCleanRestirGiGlossySecondRay` | 1 | Mixed-producer experiment: when specular mode 2 is active, seed the GI INIT page with one extra specular first-indirect ray on glossy primary surfaces. This spends more RT traversal on reflective/glossy pixels without restoring the full-screen `FirstIndirect.0d/0e` producer. Set 0 for the single-ray baseline. |
| `r_pathTracingCleanRestirGiGlossySecondRayRoughness` | 0.65 | Generous max primary roughness for the extra glossy second-ray seed. Lower toward 0.35 to match the stricter specular producer eligibility; raise only for artifact/perf A/B. |
| `r_pathTracingCleanRestirGiNeeCacheSeed` | 0 | NEE-cache seed at the temporal initial-sample step. Use with `r_pathTracingNeeCacheEnable 1`; measured faster with GI secondary NEE cache disabled. |
| `r_pathTracingCleanRestirGiNeeCacheSecondary` | 0 | Lets first-secondary GI direct lighting query the NEE cache. Current A/B: modes 1 and 2 roughly break even and showed no clear performance split, so keep off unless it gives a visible small-emissive benefit. |
