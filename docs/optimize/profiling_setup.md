...
| `r_pathTracingCleanRestirGiSpecularProducer` | 0 | Specular GI mode: 0 off, 1 full specular seed producer (`FirstIndirect.0d/0e` full-screen trace + shade), 2 one-ray mixed producer mode. Mode 2 keeps specular final-output eligibility, skips the extra full-screen specular seed producer, and lets the normal `FirstIndirect.0a` path sample a diffuse/specular mixture. |
