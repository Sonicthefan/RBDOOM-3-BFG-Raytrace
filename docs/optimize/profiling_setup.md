...
| `r_pathTracingCleanRestirGiSpatialVisibility` | 1 | Diagnostic for `CleanGI.2 SpatialReuse`: 0 accepts spatial neighbor reuse samples without the per-neighbor visibility shadow ray. Use this with the correct RT producer path to measure how much of spatial cost is visibility validation versus reservoir/light math. |
